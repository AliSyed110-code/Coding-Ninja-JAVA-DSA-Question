...
public class Palindrome_Pair {
	...
	public boolean isPalindromePair(ArrayList<String> words) {
		...
		for (String word : words) {
			add(word);
//			Add should not be mixed with searching loop, may miss some search
		}
		for (String word : words) {
			...
			if (isPalindrome(word) || search(rev.reverse().toString()) || search(rev2.reverse().toString())
					|| search(rev3.reverse().toString())) {
				return true;
			}
			int end = word.length();
			int start = 0;
